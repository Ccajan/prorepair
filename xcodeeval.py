...
def setup_gpu_environment():
    ...
    if len(sys.argv) not in (6, 7):
        print("Usage: python xcodeeval.py <model_key> <num_processes> <process_id> <gpu_id> <num_generations> [language]")
        print("参数说明:")
        ...
        print("  language(可选): 语言子集，例如 java/python/cpp/... (默认java)")
        sys.exit(1)
...
# 离线模式设置 - 默认联网；若需要离线可通过设置环境变量开启
if os.environ.get('XCODEEVAL_OFFLINE', '0') == '1':
...
# 设置显存分配策略，充分利用大显存
import os as _os
_os.environ['PYTORCH_CUDA_ALLOC_CONF'] = 'max_split_size_mb:512,expandable_segments:True'
...
def _code_fence_token(language: str) -> str:
    lang = language.lower()
    mapping = {
        'java': 'java',
        'python': 'python',
    ...
    return mapping.get(lang, 'java')


def extract_first_code(s: str, language: str) -> str:
    fence = _code_fence_token(language)
...
def main():
    ...
    # 设置prompt格式
    global BOF, EOF, model, tokenizer, USE_VLLM, LANGUAGE
    BOF, EOF = get_prompt_format(sys.argv[1])
    # 解析可选语言参数
    if len(sys.argv) >= 7 and sys.argv[6].strip():
        LANGUAGE = sys.argv[6].strip().lower()
...
def _extract_fields_from_json(json_data: dict):
    """兼容 xCodeEval 潜在字段差异，提取 code/title/description/filename"""
    # 代码字段
    code = None
    for key in [
        'buggy', 'code', 'bug_code', 'content', 'src', 'source', 'body', 'input', 'prompt_code'
    ]:
    ...
    title = None
    for key in [
        'issue_title', 'title', 'name', 'task', 'problem', 'summary'
    ]:
        if key in json_data and isinstance(json_data[key], str) and json_data[key].strip():
            title = json_data[key]
            break
    if title is None:
        title = 'xCodeEval Bug Repair'
    
    # 描述字段
    description = None
    for key in [
        'issue_description', 'description', 'desc', 'instruction', 'prompt', 'details'
    ]:
        if key in json_data and isinstance(json_data[key], str):
            description = json_data[key]
            break
    ...
    filename = None
    for key in [
        'loc', 'filename', 'file', 'path', 'filepath'
    ]:
        if key in json_data and isinstance(json_data[key], str) and json_data[key].strip():
            filename = json_data[key]
            break
    if filename is None:
        filename = 'Unknown.java'
    
    return code, title, description, filename


def process_hf_apr():
    """直接从HuggingFace加载 xCodeEval apr 子集，按语言过滤并处理"""
    import datasets
    base_fix_dir = f'xcodeeval/results/{sys.argv[1]}'
    os.makedirs(base_fix_dir, exist_ok=True)

    # 加载 apr 任务数据（使用测试集，若不可用则退化为全部/streaming）
    try:
        ds = datasets.load_dataset("NTU-NLP-sg/xCodeEval", "apr", split="test", ignore_verifications=True)
    except Exception:
        try:
            ds = datasets.load_dataset("NTU-NLP-sg/xCodeEval", "apr", split="test", streaming=True)
        except Exception:
            ds = datasets.load_dataset("NTU-NLP-sg/xCodeEval", "apr", streaming=True)

    # 语言字段可能为 'language' 或 'lang'，做兼容
    def _match_language(example: dict) -> bool:
        lang = (example.get('language') or example.get('lang') or '').strip().lower()
    ...
    for example in ds:
        try:
            if not _match_language(example):
                continue
            cnt += 1
            if cnt % int(sys.argv[2]) != int(sys.argv[3]):
                continue

            # 解析兼容字段
            code, title, description, filename = _extract_fields_from_json(example)
            if not code:
                # xCodeEval apr 可能使用 'buggy_program' 或 'program'
                for k in ['buggy_program', 'program', 'incorrect_code']:
            ...
            # 命名：优先使用 src_uid，否则使用计数器
            sample_id = str(example.get('src_uid') or example.get('uid') or cnt)
            file_name = f"{sample_id}.json"

            for e in range(int(sys.argv[5])):
                ...
                try:
                    ...
                    if os.path.exists(log_file_path):
                        ...
                        with open(log_file_path, 'r', encoding='utf-8') as log_file:
                            full = log_file.read()
                        try:
                            res = extract_first_code(full.split(EOF)[1], LANGUAGE)
                        except (IndexError, AttributeError):
                            res = None
                        if not res:
                            print("从log文件提取失败，重新生成")
                            full, res = cal(sample_id, code, title, description, filename)
                            if full is None:
                                print("修复失败，跳过")
                                continue
...
def cal_vllm(bug_id, code, title, description, filename):
    ...
    try:
        ...
        prompt = BOF + "\n# " + title + '\n' + description + '\n' + f"This is an incorrect {lang_name} code (" + filename + "):\n```" + fence + "\n" + code + "\n```\nYou are a software engineer. Can you repair the incorrect " + lang_name + " code?\n" + EOF + "\n```" + fence + "\n"
        ...
        while not ret and retry_count < max_retries:
            ...
            try:
                ret = extract_first_code(complete_text.split(EOF)[-1], LANGUAGE)
            except (IndexError, AttributeError):
                ret = None
            
            if ret:
                print(f'✅ 成功提取 {LANGUAGE} 代码 (第 {retry_count} 次尝试)', flush=True)
                print('code:', ret, flush=True)
            else:
                print(f'⚠️ 未能提取 {LANGUAGE} 代码，继续重试...', flush=True)
