...
def process(args):
    ...
    for choice in sample["model_response"]["choices"]:
...
LANG_CLUSTER_TO_LANG_COMPILER = {
    "C++": "GNU C++17",  # 只保留C++，与get_result.py一致
}
...
def main():
    ...
    parser.add_argument("--input-path", default="xcodeeval/result", type=str, help="Input folder containing model outputs")
    parser.add_argument("--model-name", default="qwen3-8b", type=str, help="Model name to evaluate")
    args = parser.parse_args()
    
    path = f'{args.input_path}/{args.model_name}/'
    for k, debug_compiler in LANG_CLUSTER_TO_LANG_COMPILER.items():
        ...
        with jsonlines.open(output_file, "w") as jwp:
            with concurrent.futures.ThreadPoolExecutor(
                max_workers=32  # 修复：减少线程数，避免资源竞争
            ) as thread_executor:
