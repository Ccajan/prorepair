...
def test(id, name, tag):
    ...
    command = "cat /data1/czj/prorepair/evalrepair-c++/result/" + tag + '/fixed' + str(id) + "/" + name + f".cpp /data1/czj/prorepair/evalrepair-c++/extend-test/{name}.cpp > tmp/{tname}.cpp && g++ --std=c++17 tmp/{tname}.cpp -lcrypto -o tmp/{tname} && timeout %d ./tmp/{tname}" % time_limit
    print(command)
...
for id in range(10):
    directory_path = f"/data1/czj/prorepair/evalrepair-c++/result/{sys.argv[1]}/fixed{id}/"
        ...
        for file_path in sorted(glob.glob(os.path.join(directory_path, '*.cpp')), reverse=False):
            ...
            if ret == 0:
                ...
                try:
                    ...
                    # 读取原始buggy代码
                    buggy_file = f"/data1/czj/prorepair/evalrepair-c++/buggy/{name}.cpp"
