...
def extract_cpp_code(text: str) -> str:
    """从生成的文本中提取C++代码（与 d4j1.py 逻辑一致）"""
    # 首先尝试匹配完整的代码块（有闭合标记）
    matches = re.findall(r'```c\+\+(.*?)```', text, re.DOTALL)
    if matches:
        return matches[0].strip()
    
    # 如果没有闭合标记，尝试匹配从 ```c++ 开始到字符串结尾或到下一个 ``` 的内容
...
def generate_with_vllm(llm, prompt: str):
    """使用 vLLM 生成代码"""
    ...
    # 根据模型格式提取生成的 C++ 代码部分
    try:
        ret = extract_cpp_code(full_text.split(EOF)[-1])
    except (IndexError, AttributeError):
        ret = extract_cpp_code(full_text)
...
def reextract_code_from_log(log_file_path):
    """从log文件中提取第二个C++代码块（第一个是prompt，第二个是模型输出）"""
    try:
        with open(log_file_path, 'r', encoding='utf-8') as f:
            log_content = f.read()
        
        # 查找所有C++代码块
        cpp_blocks = re.findall(r'```c\+\+(.*?)```', log_content, re.DOTALL)
...
def generate_fix(code: str, filename: str, prompt_dir: str):
    """生成代码修复"""
    prompt_suffix = get_prompt_suffix(filename, prompt_dir)
    prompt = f"{BOF} This is an incorrect code ({filename}):\n```c++\n{code}\n```\nYou are a software engineer. Can you repair the incorrect code? Please provide ONLY the corrected C++ code without any explanations or comments outside the code.\n{EOF}\n```c++\n{prompt_suffix}"
    print(prompt, flush=True)
    
    return generate_with_vllm(llm, prompt)
...
for file_path in sorted(Path(base_dir).rglob('*.cpp'), reverse=True):
    ...
    for e in range(num_generations):
        ...
        # 生成修复代码
        result = generate_fix(content, file_name, prompt_dir)
